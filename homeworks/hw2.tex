...
\paragraph{1.} The distance function of logistic regression was defined as 
\begin{align*}
    D(M^*(\vx), M, \vx) = -(M^*(\vx) \log M(\vx) + (1-M^*(\vx)) \log (1- M(\vx))).
\end{align*}
Derive its gradient with respect to the weight vector $\vw$ step-by-step.

\vspace{30mm}

\paragraph{2.} After replacing the label set from $\left\{ 0,1 \right\}$ to
