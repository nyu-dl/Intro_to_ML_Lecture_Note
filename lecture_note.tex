...
\begin{align*}
    \|\vy^* - \vy\|_2 = \sqrt{\sum_{k=1}^q (y^*_k - y_k)^2}.
\end{align*}
Of course, this is not the only way, and a straightforward generalization is to
consider $L_p$ norm of which Euclidean distance is a special case with $p=2$:
\begin{align*}
    \|\vy^* - \vy\|_p = \sqrt[p]{\sum_{k=1}^q (y^*_k - y_k)^p}.
\end{align*}
