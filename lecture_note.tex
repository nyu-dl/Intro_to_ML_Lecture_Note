...
\begin{align}
    ...
    D(M^*(\vx), M, \vx) = \|M^*(\vx) - M(\vx)\|_2^2 = 
    \sum_{k=1}^q (y^*_k - y_k)^2
\end{align}
...
\begin{align*}
    \text{Moment}-p(X) = \sum_{x \in \Omega} (x - \E\left[ X \right])^p p(x).
\end{align*}
...
variance $\sigma^2$. Mathematically, we express this prior distribution by
\begin{align*}
    p(\theta) = \prod_{i=1}^{|\theta|} \frac{1}{\sqrt{2\pi}} \exp \left(
    -\frac{1}{2} \theta_i^2 \right),
\end{align*}
...
\begin{align}
    ...
    \log p(\theta) = 
    \sum_{i=1}^{|\theta|} 
    -\frac{1}{2} \theta_i^2 - \log \sqrt{2\pi}.
\end{align}
...
\begin{align*}
    ...
    \underbrace{
        \sum_{i=1}^{d} 
        \sum_{j=1}^{q}
        -\frac{1}{2} w_{ij}^2 - \log \sqrt{2\pi}
    }_{=\log p(\theta)}
    - 
    \log p(D).
\end{align*}

\todo{Connection to the regularization}
